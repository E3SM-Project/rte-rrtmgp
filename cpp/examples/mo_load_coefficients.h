...
void load_and_init(
  ...
  GasConcsK<RealT, LayoutT, DeviceT> const &available_gases)
{
  conv::SimpleNetCDF io;
  io.open(filename , conv::NETCDF_MODE_READ);

  using int1dh_t  = Kokkos::View<int*,   LayoutT, HostDevice>;
  using int2dh_t  = Kokkos::View<int**,  LayoutT, HostDevice>;
  using int3dh_t  = Kokkos::View<int***, LayoutT, HostDevice>;

  using real1dh_t = Kokkos::View<RealT*,    LayoutT, HostDevice>;
  using real2dh_t = Kokkos::View<RealT**,   LayoutT, HostDevice>;
  using real3dh_t = Kokkos::View<RealT***,  LayoutT, HostDevice>;
  using real4dh_t = Kokkos::View<RealT****, LayoutT, HostDevice>;

  using char2dh_t = Kokkos::View<char**, LayoutT, HostDevice>;

  using bool1dh_t = Kokkos::View<bool*, LayoutT, HostDevice>;
  ...
  real3dh_t vmr_ref;
  real4dh_t kmajor;
  int2dh_t  minor_limits_gpt_lower;
  int2dh_t  minor_limits_gpt_upper;
  bool1dh_t minor_scales_with_density_lower;
  bool1dh_t minor_scales_with_density_upper;
  bool1dh_t scale_by_complement_lower;
  bool1dh_t scale_by_complement_upper;
  int1dh_t  kminor_start_lower;
  int1dh_t  kminor_start_upper;
  real3dh_t kminor_lower;
  real3dh_t kminor_upper;
  real3dh_t rayl_lower;
  real3dh_t rayl_upper;

  // Read in strings
  char2dh_t tmp;
  tmp = char2dh_t();  io.read( tmp , "gas_names"         );  gas_names         = char2d_to_string1d(tmp);
  tmp = char2dh_t();  io.read( tmp , "gas_minor"         );  gas_minor         = char2d_to_string1d(tmp);
  tmp = char2dh_t();  io.read( tmp , "identifier_minor"  );  identifier_minor  = char2d_to_string1d(tmp);
  tmp = char2dh_t();  io.read( tmp , "minor_gases_lower" );  minor_gases_lower = char2d_to_string1d(tmp);
  tmp = char2dh_t();  io.read( tmp , "minor_gases_upper" );  minor_gases_upper = char2d_to_string1d(tmp);
  tmp = char2dh_t();  io.read( tmp , "scaling_gas_lower" );  scaling_gas_lower = char2d_to_string1d(tmp);
  tmp = char2dh_t();  io.read( tmp , "scaling_gas_upper" );  scaling_gas_upper = char2d_to_string1d(tmp);
